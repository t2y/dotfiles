...
description: 日本語で書く markdown 全般の文章規範。整形（一文一行、引用ブロック、脚注、コラム記法）、段落と論証の構成（パラグラフライティング）、論証の厳密さ（ツッコミどころの除去）、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除を定める。日本語で技術書の章、草稿、記事、解説文、README、設計ドキュメント、設計プラン（~/.claude/plans/ 配下）、GitLab / GitHub の issue・merge request・pull request の説明やコメントを書くとき、または推敲・リライトするときに使用する。
---

# 日本語技術文書の文章規範

日本語で文章を書く・推敲するときは、以下の規範に従う。

## 適用対象

日本語で書く markdown 全般に適用する。

- 技術書の章、草稿、記事、解説文
- README、設計ドキュメント、仕様書など、リポジトリ内の日本語 markdown
- 設計プラン（`~/.claude/plans/` 配下のプランファイル）
- GitLab の issue、merge request の説明とコメント（GitHub の issue、pull request も同じ）
- コミットメッセージの本文（日本語で書く場合）

いずれも Write / Edit する前にこの規範を読み、書いたあとは同じ規範で推敲する。

短い実務文書（README、issue、merge request の説明）でも、「整形」「論証の厳密さ」「LLM っぽい表現の禁止」「冗長の排除」「見出しの付け方」「読者への誠実さ」はそのまま効く。
実務文書ほど読み手の時間が限られるので、冗長の排除と LLM 口調の禁止はとくに厳しく適用する。

一方、書籍原稿に固有の作法は実務文書に持ち込まない。
脚注（`[^ラベル]`）、コラム見出し、章節をまたぐ前方参照、山場の演出がこれにあたる。
issue や merge request の本文で脚注に降ろすくらいなら、その補足は削るか、括弧で本文に収める。

README や手順書の操作説明では、読者への呼びかけや命令形を使ってよい。
「論証の中で読者を『あなた』と呼ばない」は、論証を組み立てる文章に対する規範であり、手順の指示には適用しない。
